...
impl Model {
    ...
    /// use std::fs::File;
    ///
    /// let f = File::open("model.zms").unwrap();
    /// let _ = model::from_file(f);
    pub fn from_file(file: File) -> Result<Model> {
        let mut zms = Model::new();
        zms.load(file)?;
        Ok(zms)
    }

    /// Construct a `Model` from a path slice
    ///
    /// # Usage
    /// ```rust,no_run
    /// use roseon::model::Model;
    ...
    /// use roseon::model::Model;
    ///
    /// let f = File::open("example.zms").unwrap();
    /// let mut zms = Model::new();
    /// zms.load(f).unwrap();
    ///
    /// for _ in zms.vertices {
    ///     // Do something
    /// }
    /// ```
    ...
    /// use roseon::model::Model;
    ///
    /// let in_file = File::open("in.zms").unwrap();
    /// let out_file = File::open("out.zms").unwrap();
    ///
    /// let mut zms = Model::from_file(in_file).unwrap();
    ///
    /// // Do something with file
    /// zms.save(out_file).unwrap();
    /// ```
...
mod tests {
    ...
    fn model_load() {
        ...
        let model1 = Model::from_path(&file1).unwrap();
        ...
        let model2 = Model::from_path(&file2).unwrap();
        ...
        let model3 = Model::from_path(&file3).unwrap();
    ...
    fn model_save() {
        ...
        for zms_file in [file1, file2, file3].iter() {
            ...
            let mut orig_zms = Model::from_path(&zms_file).unwrap();

            let mut buffer: Vec<u8> = Vec::new();
            buffer.resize(zms_size as usize, 0u8);

            let mut cursor = Cursor::new(buffer);
            orig_zms.save_writer(&mut cursor).unwrap();

            cursor.set_position(0);

            let mut new_zms = Model::new();
            new_zms.load_reader(&mut cursor).unwrap();
